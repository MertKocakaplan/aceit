...
import { statsAPI, studySessionsAPI, studyPlanAPI, spacedRepetitionAPI } from '../../api';
import {
  Clock,
  Target,
  TrendingUp,
...
import { ThemeToggle, DashboardHeader } from '../../ui';
...
const Dashboard = () => {
  ...
  const [loadingDueTopics, setLoadingDueTopics] = useState(true);
  ...
  const fetchDueTopics = async () => {
    try {
      setLoadingDueTopics(true);
      const response = await spacedRepetitionAPI.getDueTopics(10);
      setDueTopics(response.data || []);
    } catch (error) {
      logger.error('Due topics fetch error:', error);
      setDueTopics([]);
    } finally {
      setLoadingDueTopics(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-secondary-100 via-neutral-50 to-secondary-50 dark:from-neutral-950 dark:via-neutral-900 dark:to-neutral-950 transition-colors duration-300 relative overflow-hidden">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        {/* Due Reviews */}
        <DueReviewsCard dueTopics={dueTopics} loading={loadingDueTopics} />
